...
import 'package:my_shop/Service/Users.dart';
  ...
  Future<bool> signUp()async{
    try{
      ...
      await _auth.createUserWithEmailAndPassword(email: email.text.trim(), password: password.text.trim()).then((result){
        _firestore.collection('Customers').document(result.user.uid).setData({
          'name':name.text,
          'email':email.text,
          'id':result.user.uid,
          "likedProduct": [],
  ...
  Future<bool> addToCard({ProductModel product, int quantity})async{
    ...
    try{
      ...
      Map cartItem ={
        ...
        "price": product.price-(product.price*product.sale/100).round(),
        "quantity": quantity
      };
      print("CART ITEMS: ${cart.toString()}");
